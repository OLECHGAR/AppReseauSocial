...
public class TestEnvoiSurZonePartage {
	...
	try {
	    ...
	    u = new Utilisateur("", "Bernard", "", "Bernard", "", "", "", null);
	    u2 = new Utilisateur("", "Gerard", "", "Gerard", "", "", "", null);
	    Utilisateur u3 = new Utilisateur("", "", "", "", "", "", "", null);
	    String nomZone = "Test";
		...
		if ("q".equals(input)) {
			...
			System.out.println("("
				+ zone1.getTransferables().get(i).timeMessage()
				+ ") "
				+ ((Utilisateur) zone1.getTransferables()
					.get(i).getProprietaire()).getNom()
		...
		if (nbRequetes % 2 == 0) {
		    ...
		    if (zone1.getPrivacy()) {
			Iterator<SalonDiscussion> it = u2
				.getZonesInteractions().iterator();
			while (it.hasNext()) {
			    if (it.next().equals(zone1)) {
				new Envoi<Utilisateur>(input, u2, zone1,
					"texte");
			    }
			}
		    } else {
		...
		nbRequetes++;
		System.out.println("("
			+ zone1.getTransferables().get(zone1.size() - 1)
				.timeMessage()
			+ ") "
			+ ((Utilisateur) zone1.getTransferables()
				.get(zone1.size() - 1).getProprietaire())
				.getNom() + " : "
